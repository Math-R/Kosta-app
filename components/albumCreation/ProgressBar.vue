<template>
  <div class="progressContainer my-8">
    <div class="stepBar relative h-05">
      <div
        :class="[
          'step-indicator h-5 w-5 rounded-full  absolute -top-50 ',
          index <= currentStep ? 'bg-red' : 'bg-red-lighter',
        ]"
        :style="{ left: stepPosition(index) + '%' }"
        v-for="(step, index) in steps"
        :key="step"
      ></div>
      <div
        class="progress absolute top-0 left-0 bg-red h-05"
